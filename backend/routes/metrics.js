...
import { getHealth } from '../collector/index.js'
...
export const createMetricsRouter = (monitor) => {
  ...
  router.get('/summary', (req, res) => {
    ...
    res.json({
      ...
      rx:     latest.network?.rx_sec ?? 0,
      tx:     latest.network?.tx_sec ?? 0,
      procs:  latest.processes?.length ?? 0,
      ports:  latest.ports?.length     ?? 0,
